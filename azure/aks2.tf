resource "azurerm_resource_group" "unicorns-aks2" {
  name = "unicorns-aks2-rg"
  location = "West US 3"
}

locals {
  aks2_kubernetes_version = "1.30.10"
}

resource "azurerm_kubernetes_cluster" "unicorns-aks2" {
  name                = "unicorns-aks2"
  location            = azurerm_resource_group.unicorns-aks2.location
  resource_group_name = azurerm_resource_group.unicorns-aks2.name
  dns_prefix          = "unicorns-aks2"

  kubernetes_version = local.aks2_kubernetes_version

  default_node_pool {
    name       = "default"
    node_count = 1

    orchestrator_version = local.aks2_kubernetes_version
    ...
    # 2 vCPU, 4 GiB RAM. 8 GiB temp disk. 30 GiB cache.
    # vm_size    = "Standard_B2s"
    # 2 vCPU, 8 GiB RAM. No temp disk. Does not support ephemeral OS disk. $525.60/year in West US 3.
    vm_size    = "Standard_B2ps_v2"
    # 2 vCPU, 8 GiB RAM. 16 GiB temp disk. 30 GiB cache.
...
resource "azurerm_kubernetes_cluster_node_pool" "aks2spot1" {
  name = "aks2spot1"
  
  orchestrator_version = local.aks2_kubernetes_version
  kubernetes_cluster_id = azurerm_kubernetes_cluster.unicorns-aks2.id
  ...
  # vm_size = "Standard_D2ads_v5" # 2vCPU, 8GiB RAM, 75 GiB temp disk, 0 GiB cache.
  ...
  # vm_size = "Standard_D2pds_v5" # 2vCPU, 8GiB RAM, 75 GiB temp disk, 50 GiB cache.
  vm_size = "Standard_D2ps_v6" # 2vCPU, 8GiB RAM, no temp disk. Spot price $86.11/year in West US 3. Doesn't support ephemeral OS disk.
  # vm_size = "Standard_E2pds_v5" # 2vCPU, 16GiB RAM, 75 GiB temp disk, 50 GiB cache. Does not support spot VMs
  # vm_size = "Standard_E2ps_v5" # 2vCPU, 16GiB RAM, no temp disk, 50 GiB cache. Doesn't support ephemeral OS disk.
  # vm_size = "Standard_E2a_v4" # 2vCPU, 16GiB RAM, 50 GiB temp disk, no cache.
  # vm_size = "Standard_E2as_v4" # 2vCPU, 16GiB RAM, 32 GiB temp disk, 50 GiB cache.
  ...
  spot_max_price = format("%.5f", (86.11 * local.annual_spot_cost_multiplier) / 12 / local.azure_hours_per_month) # max price per hour. Rounding because the provider only accepts 5 decimal places.
  node_taints = [
...
output "aks2_cluster_name" {
  value = azurerm_kubernetes_cluster.unicorns-aks2.name
}

output "aks2_kube_config" {
  value     = azurerm_kubernetes_cluster.unicorns-aks2.kube_config_raw
  sensitive = true
}
